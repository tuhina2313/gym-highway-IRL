    ...
    def train(self, rwd_weight=None):
        ...
        for i_episode in tqdm.tqdm(range(self.num_episodes)):
            ...
            for t in count():
                ...
                s_dummy = self.env.reset()
                next_state, reward, done, _ = self.env.step(action.item())
                
                observation_tuple = []
                observation_tuple.append(next_state[0].tolist())
                observation_tuple.append(next_state[1].tolist())

                reward = self.feature_func(observation_tuple)
