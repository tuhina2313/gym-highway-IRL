...
def irl(features, feature_dim, n_actions, discount, transition_probability, trajectories, epochs, learning_rate):

    n_states, d_states = feature_matrix.shape

    # Initialise weights.
    #alpha = rn.uniform(size=(d_states,))
    alpha = np.zeros((feature_dim,), dtype=float)

    # Calculate the feature expectations.
    feature_expectations = find_feature_expectations(features, trajectories)
    # Gradient descent on alpha.
    for i in range(epochs):
        # print("i: {}".format(i))
        r = features.dot(alpha)
        expected_svf = find_expected_svf(n_states, r, n_actions, discount, transition_probability, trajectories)
        grad = feature_expectations - features.T.dot(expected_svf)

        alpha += learning_rate * grad

    return features.dot(alpha).reshape((n_states,))
